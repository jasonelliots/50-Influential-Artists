...
*/
function getArtistByIndex(a, id) {
  return `The artist at index ${id} is ${a[id].name}`; 
}
...
function get20s(a){
  ...
  for (i = 0; i < a.length; i++){
    if (Math.min(a[i].years) < 2000 && Math.min(a[i].years) >= 1900){
      twenties.push(a[i].name); 
    }
  }
...
function checkArtist(str){
  for (i = 0; i < artists.length; i++){
    if (artists[i].name === 'str'){
      return true;
    } else {
      return false; 
    }
